...
RSpec.describe('Navigation Test', type: :feature) do
  ...
  it 'does not have navigation on sign out page' do
    visit root_path
    expect(page).not_to(have_content('Home Page'))
  end
  it 'has navigation on every other page' do
    visit '/admins/auth/google_oauth2/callback'
...
RSpec.describe('Deleting a user', type: :feature) do
  let!(:user) { User.create(first_name: 'Joe', last_name: 'Shmoe', uin: '730303036', phone_number: '8324344445', email: 'student@tamu.edu', dob: '2003-10-10', points: '3', role_id: '3') }
  let!(:user2) { User.create(first_name: 'Joe', last_name: 'Shmoe', uin: '730304036', phone_number: '8324344445', email: 'student2@tamu.edu', dob: '2003-10-10', points: '3', role_id: '2') }
...
RSpec.describe('My Personal Calendar:', type: :feature) do
  let!(:user) { User.create(first_name: 'Joe', last_name: 'Shmoe', uin: '730303036', phone_number: '8324344445', email: 'student@tamu.edu', dob: '2003-10-10', points: '3', role_id: '1') }
  let!(:event1) { Event.create(start: Date.current, end: Date.current + 1.day, type_id: '3', status: 'ongoing', name: 'Event 1', description: 'This is Event 1') }
...
RSpec.describe('Main Calendar:', type: :feature) do
  ...
  it 'automaticallies show the view based on the current month and year' do
    visit '/admins/auth/google_oauth2/callback'
    visit root_path
    expect(page).to(have_content(Date.current.strftime('%B %Y')))
  end

  it 'displays the next month accordingly when the next button is clicked' do
    visit '/admins/auth/google_oauth2/callback'
    visit root_path
  ...
  it 'displays the previous month accordingly when the previous button is clicked' do
    visit '/admins/auth/google_oauth2/callback'
    visit root_path
    current_month_year = Date.current.strftime('%B %Y')
    click_on 'Previous'
  ...
  describe '#perform' do
    ...
    it 'sets the points attribute to 0 for all users' do
      expect do
        ...
        user2.reload
        user3.reload
      end.to(change { user1.points }.from(10).to(0)
